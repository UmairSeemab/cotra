  ...
  tagList(
    ...
    conditionalPanel(condition = "input.type_import == 'flowr'", ns = ns,
                     fluidRow(
                       ...
                       box(title = p("Import Rds file", actionButton(ns("HELP_DATA_IMPORT_CoTRA"), "", icon = icon("question-circle"), class = "btn-xs")),
                           width = 4, height = NULL,  
                           solidHeader = TRUE, status = "primary", 
...
Import <- function(input, output, session, rval) {
  ...
  observeEvent(input[["HELP_DATA_IMPORT_CoTRA"]], {
    
    showModal(modalDialog(
      title = HTML(help_infos[which(help_infos$key == "HELP_DATA_IMPORT_CoTRA"),"title"]),
      HTML(help_infos[which(help_infos$key == "HELP_DATA_IMPORT_CoTRA"),"value"]),
      easyClose = TRUE,
      footer = modalButton("OK")
    ))
  ...
  observeEvent(input$flowrfile, {
    ...
    if(all(names(rds) %in% names(rval))){
      ...
      showModal(modalDialog(
        ...
        "Your file doesn't contain a CoTRA object.",
        easyClose = TRUE,
        footer = modalButton("OK")
      ))
    }
